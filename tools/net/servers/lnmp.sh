...
sed -i '1,$s/;error_log = log\/php-fpm.log/error_log = log\/php-fpm.log/g' /usr/local/etc/php-fpm.conf
if [ $? -ne 0 ]; then
	echo 'Failed to optimize php-fpm.conf'
	return 1
fi

sed -i '1,$s/;log_level = notice/log_level = warning/g' /usr/local/etc/php-fpm.conf
if [ $? -ne 0 ]; then
	echo 'Failed to optimize php-fpm.conf'
	return 1
fi
